...
|---|---|---|
| sway/Hyprland/niri layout | **`kukaku`** (NET-NEW; `geom` extracted from mado's `float`) | ABSENT — `handlers.rs:109` maps every toplevel at `(0,0)`; `move_request`/`resize_request` are empty bodies (`:157`, `:160`) |
| i3-msg / hyprctl keybinding layer | **`awase`** `BindingMap`/`KeyMode`/`Action`/`Condition`/`Banzuke`/`KeyRepeatGate` | ABSENT-runtime — omoya uses **1 of awase's 12 modules** (`Reserved::fleet_linux()` + a keysym adapter). **There is no chord path at all: a program starts only via `omoya -- <cmd>`** |
...
| **`kukaku`** (区画) | a parcel — a space partitioned into disjoint plots | **free** (0/2441, no repo). Reads as exactly its job |
