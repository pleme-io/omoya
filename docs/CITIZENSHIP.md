...
| input **hotplug** (`libudev`) | **SHIPPED** — NET-NEW `uevent::UeventMonitor` on a **netlink group-2 (udev monitor) socket**, consumed by `evdev_backend` with real `Hotplug::Added`/`Removed` arms. Group 2 and not group 1 is the load-bearing detail: the kernel creates the devtmpfs node in `drivers/base/core.c` **before** emitting the uevent, so inotify `IN_CREATE` and the kernel-group netlink both fire too early for logind's `TakeDevice` to succeed. This row said ABSENT after the code landed — the stale-model shape this ledger exists to prevent. | only-mitigated (C2) |
| client **GPU** buffers (`zwp_linux_dmabuf_v1`) | **SHIPPED — ADVERTISED.** `drm.rs` creates the global from `renderer.dmabuf_formats()` (never a hand-written list, so the promise and the capability cannot drift), with `DmabufHandler` + `delegate_dmabuf!`. Witnessed on vkms: `zwp_linux_dmabuf_v1 advertised — clients may deliver GPU buffers formats=2`. Version **3, not feedback**: feedback's `main_device` means "the device the compositor renders on", and omoya renders on the CPU into dumb buffers — naming a render node would be a claim it cannot back. **LINEAR only**, because nuri MAPS the buffer and a tiled modifier describes a layout only a GPU can decode; a CPU blitter reading it paints structured noise rather than failing. Zero-copy import is still a copy (`pending-nuri-dmabuf-zerocopy`). | only-mitigated (C2) |
