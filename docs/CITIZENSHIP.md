...
| input **hotplug** (`libudev`) | **ABSENT.** `evdev_backend` reads `/dev/input` once at startup and never watches it — no inotify, no netlink. A keyboard plugged in after the compositor starts is invisible until restart. libudev did this and it was lost in the swap. `pending-omoya-input-hotplug` | only-mitigated (C6) |
